...
def build_ashtakavarga(rows: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for target in CLASSICAL:
        ...
        for index, value in enumerate(bindus):
            sav[index] += value
        bav_rows.append({"key": target, "planet": PLANET_NAMES_BG[target], "bindus": bindus, "total": sum(bindus)})
    return {
...
def build_strength_bundle(*, rows: dict[str, dict[str, object]]) -> dict[str, object]:
    return {
        "relationships": build_relationship_table(rows),
        "ashtakavarga": build_ashtakavarga(rows),
    }
